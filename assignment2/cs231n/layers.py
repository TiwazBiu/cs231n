...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    num_train = x.shape[0]
    flatten_x = x.reshape(num_train,-1)
...
def batchnorm_backward(dout, cache):
    ...
    gamma,std,xc = cache 
    dxc = dout * gamma
    dvar = -0.5*np.sum(dxc * xc / std**2,axis=0)
    dmu = -np.sum(dxc/std + 2*dvar*xc*std/N,axis=0)

    dx = dxc/std + dvar*(2*xc*std/N) + dmu/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    gamma,std,xc = cache 
    dxc = dout * gamma
    dx = 1/std*(dxc - np.mean(dxc,axis=0)-xc*np.mean(xc*dxc,axis=0))
    dbeta = np.sum(dout,axis=0)
    dgamma = np.sum(dout * xc,axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    xp = x.transpose((1,0,2,3)).reshape((C,-1)).T
    preout,cache = batchnorm_forward(xp, gamma, beta, bn_param)
    out = preout.T.reshape((C,N,H,W)).transpose((1,0,2,3))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dpreout = dout.transpose((1,0,2,3)).reshape((C,-1)).T
    dxp,dgamma,dbeta = batchnorm_backward(dpreout,cache)
    dx = dxp.T.reshape((C,N,H,W)).transpose((1,0,2,3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
